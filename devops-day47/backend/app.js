...
if (missingEnvVars.length > 0) {
  console.error('Day 47 backend failed to start.');
...
if (!CHANGE_RECORD_ID.startsWith('DAY47-CHANGE-')) {
...
function createRequestId() {
  return `day47-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
...
app.get('/api/message', (req, res) => {
  res.status(200).json({
    message: 'Hello from Day 47 backend API',
    service: SERVICE_NAME,
    environment: APP_ENV,
    deployTarget: DEPLOY_TARGET,
    version: APP_VERSION,
...
app.get('/api/stability', (req, res) => {
  ...
  if (stabilityForcedToFail) {
    ...
    return res.status(500).json({
      ...responseBody,
      error: 'Day 47 forced stability failure'
    });
  }
...
app.get('/api/error-test', (req, res) => {
  ...
  res.status(500).json({
    error: 'Day 47 controlled error test',
    service: SERVICE_NAME,
    deployTarget: DEPLOY_TARGET,
    requestId: req.requestId,
    ...buildMetadata()
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Approval required: ${APPROVAL_REQUIRED}`);
