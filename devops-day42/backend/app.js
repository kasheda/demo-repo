...
if (missingEnvVars.length > 0) {
  console.error('Day 42 backend failed to start.');
...
function createRequestId() {
  return `day42-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
...
app.get('/api/message', (req, res) => {
  res.status(200).json({
    message: 'Hello from Day 42 backend API',
    service: SERVICE_NAME,
    environment: APP_ENV,
    deployTarget: DEPLOY_TARGET,
    version: APP_VERSION,
...
app.get('/api/error-test', (req, res) => {
  ...
  res.status(500).json({
    error: 'Day 42 controlled error test',
    service: SERVICE_NAME,
    deployTarget: DEPLOY_TARGET,
    requestId: req.requestId,
    ...buildMetadata()
