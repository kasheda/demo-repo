...
if (missingEnvVars.length > 0) {
  console.error('Day 43 backend failed to start.');
...
function createRequestId() {
  return `day43-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
...
app.get('/api/message', (req, res) => {
  res.status(200).json({
    message: 'Hello from Day 43 backend API',
    service: SERVICE_NAME,
    environment: APP_ENV,
    deployTarget: DEPLOY_TARGET,
    version: APP_VERSION,
...
app.get('/api/error-test', (req, res) => {
  ...
  res.status(500).json({
    error: 'Day 43 controlled error test',
    service: SERVICE_NAME,
    deployTarget: DEPLOY_TARGET,
    requestId: req.requestId,
    ...buildMetadata()
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Stability window seconds: ${STABILITY_CHECK_WINDOW_SECONDS}`);
  console.log(`Stability interval seconds: ${STABILITY_CHECK_INTERVAL_SECONDS}`);
