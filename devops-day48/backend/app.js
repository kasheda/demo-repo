...
if (missingEnvVars.length > 0) {
  console.error('Day 48 backend failed to start.');
...
if (!['true', 'false'].includes(METRICS_ENABLED)) {
  fail(`Invalid METRICS_ENABLED value: ${METRICS_ENABLED}`);
}

if (!['normal', 'rollback'].includes(ROLLBACK_MODE)) {
  fail(`Invalid ROLLBACK_MODE value: ${ROLLBACK_MODE}`);
}

if (!Number.isInteger(STABILITY_CHECK_WINDOW_SECONDS) || STABILITY_CHECK_WINDOW_SECONDS <= 0) {
  fail(`Invalid STABILITY_CHECK_WINDOW_SECONDS value: ${process.env.STABILITY_CHECK_WINDOW_SECONDS}`);
}

if (!Number.isInteger(STABILITY_CHECK_INTERVAL_SECONDS) || STABILITY_CHECK_INTERVAL_SECONDS <= 0) {
  fail(`Invalid STABILITY_CHECK_INTERVAL_SECONDS value: ${process.env.STABILITY_CHECK_INTERVAL_SECONDS}`);
}

if (!['true', 'false'].includes(FORCE_STABILITY_FAILURE)) {
  fail(`Invalid FORCE_STABILITY_FAILURE value: ${FORCE_STABILITY_FAILURE}`);
}

if (!['none', 'prodlike'].includes(PROMOTION_TARGET)) {
  fail(`Invalid PROMOTION_TARGET value: ${PROMOTION_TARGET}`);
}

if (!['true', 'false'].includes(PROMOTION_APPROVED)) {
  fail(`Invalid PROMOTION_APPROVED value: ${PROMOTION_APPROVED}`);
}

if (!CHANGE_RECORD_ID.startsWith('DAY48-CHANGE-')) {
  fail(`Invalid CHANGE_RECORD_ID value: ${CHANGE_RECORD_ID}`);
}

if (!['true', 'false'].includes(RELEASE_NOTES_REQUIRED)) {
  fail(`Invalid RELEASE_NOTES_REQUIRED value: ${RELEASE_NOTES_REQUIRED}`);
}

if (!['draft', 'generated', 'not-required'].includes(RELEASE_NOTES_STATUS)) {
  fail(`Invalid RELEASE_NOTES_STATUS value: ${RELEASE_NOTES_STATUS}`);
}

if (!['true', 'false'].includes(APPROVAL_REQUIRED)) {
  fail(`Invalid APPROVAL_REQUIRED value: ${APPROVAL_REQUIRED}`);
}

if (!['pending', 'approved', 'not-required'].includes(APPROVAL_STATUS)) {
  fail(`Invalid APPROVAL_STATUS value: ${APPROVAL_STATUS}`);
}
...
function createRequestId() {
  return `day48-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
...
function logInfo(message, details = {}) {
  if (['info', 'debug'].includes(LOG_LEVEL)) {
    console.log(JSON.stringify({
      ...
      approvalStatus: APPROVAL_STATUS,
      deploymentWindowStatus: DEPLOYMENT_WINDOW_STATUS,
      changeFreezeActive: CHANGE_FREEZE_ACTIVE,
      message,
      ...details
    }));
  }
...
app.get('/api/message', (req, res) => {
  res.status(200).json({
    message: 'Hello from Day 48 backend API',
    service: SERVICE_NAME,
    environment: APP_ENV,
    deployTarget: DEPLOY_TARGET,
    version: APP_VERSION,
...
app.get('/api/stability', (req, res) => {
  ...
  if (stabilityForcedToFail) {
    ...
    return res.status(500).json({
      ...responseBody,
      error: 'Day 48 forced stability failure'
    });
  }
...
app.get('/api/error-test', (req, res) => {
  ...
  res.status(500).json({
    error: 'Day 48 controlled error test',
    service: SERVICE_NAME,
    deployTarget: DEPLOY_TARGET,
    requestId: req.requestId,
    ...buildMetadata()
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Deployment window status: ${DEPLOYMENT_WINDOW_STATUS}`);
  console.log(`Change freeze active: ${CHANGE_FREEZE_ACTIVE}`);
